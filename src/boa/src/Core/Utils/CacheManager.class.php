...
class CacheManager {
	...
	/**
	 * Create an CacheManager instance
	...
	public static function getItem($pluginId, $filepath, $dataCallback=null, $idComputerCallback = null){
		if($dataCallback == null){
			$dataCallback = array("BoA\Core\Utils\CacheManager", "simpleCopy");
		}
		return new CacheManager($pluginId,$filepath, $dataCallback, $idComputerCallback);
	}
	...
	public static function clearItem($pluginId, $filepath){
		$inst = new CacheManager($pluginId,$filepath, false);
		Logger::debug("SHOULD REMOVE ".$inst->getId());
     ...
     * @return void
     */
	public function __construct($pluginId, $filepath, $dataCallback, $idComputerCallback = NULL){
		$this->cacheDir = (defined('APP_SHARED_CACHE_DIR')?APP_SHARED_CACHE_DIR:APP_CACHE_DIR);
	...
	public function getData(){
		if(!$this->hasCachedVersion()){
			Logger::debug("caching data", $this->dataCallback);
