...
class UserSelection
{
	...
	function initFromArray($array)
	{
		...
		{
			$this->files[] = Utils::decodeSecureMagic($array[$this->varPrefix]);
			$this->isUnique = true;
			//return ;
		}
		if(isSet($array[$this->varPrefix."_0"]))
		{
			...
			{
				$this->files[] = Utils::decodeSecureMagic($array[$this->varPrefix."_".$index]);
				$index ++;
			}
			$this->isUnique = false;
	...
	function getZipPath($decode = false){
		if($decode) return Utils::decodeSecureMagic($this->zipFile);
