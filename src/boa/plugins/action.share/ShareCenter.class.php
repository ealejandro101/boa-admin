...
require_once("PublicletCounter.class.php");
...
class ShareCenter extends Plugin{
	...
	protected function parseSpecificContributions(&$contribNode){
		...
		if(isSet($this->actions["share"])){
			...
			if($downloadFolder == ""){
				...
				Logger::debug("Disabling Public links, $downloadFolder is not writeable!", array("folder" => $downloadFolder, "is_dir" => is_dir($downloadFolder),"is_writeable" => is_writable($downloadFolder)));
				$disableSharing = true;
			}else{
    ...
    function switchAction($action, $httpVars, $fileVars){
        ...
        switch($action){
            ...
            case "share":
                ...
                if($metadata != null && $node->hasMetaStore()){
                    $node->setMetadata(
                        "app_shared",
                        $metadata,
                        true,
            ...
            case "load_shared_element_data":
                ...
                if(count($metadata)){
                    ...
                    if($elementType == "file"){
                        ...
                        if($this->watcher != false){
                            $elementWatch = $this->watcher->hasWatchOnNode(
                                new ManifestNode($this->baseProtocol."://".$repoId."/"),
                                AuthService::getLoggedUser()->getId(),
                        ...
                        $jsonData = array(
                            ...
                            "element_watch" => $elementWatch,
                            "repository_url"=> Utils::detectServerURL(true)."?goto=". $repo->getSlug() ."/"
                        );
    ...
    function nodeSharedMetadata(&$node){
        ...
        if(count($metadata)){
            ...
            if($eType == "minisite") $merge["app_shared_minisite"] = $metadata["minisite"];
	...
	public function updateNodeSharedData($oldNode/*, $newNode = null, $copy = false*/){
        ...
        if(count($metadata) && !empty($metadata["element"])){
            ...
            try{
                ...
                $oldNode->removeMetadata("app_shared", true, APP_METADATA_SCOPE_REPOSITORY, true);
            }catch(\Exception $e){
                Logger::logAction("ERROR : ".$e->getMessage(), $e->getTrace() );
            }
        }
    }
    ...
    */
    function writePubliclet(&$data, $accessDriver, $repository)
    {
        ...
        if($accessDriver->hasMixin("credentials_consumer")){
        	$cred = Credential::tryLoadingCredentialsFromSources(array(), $repository);
    ...
    function buildPublicDlURL(){
        ...
        if($dlURL != ""){
            ...
            return str_replace("\\", "/", rtrim($fullUrl, "/").rtrim(str_replace(APP_INSTALL_PATH, "", $downloadFolder), "/"));
        }
    }

    function computeMinisiteToServerURL(){
        $minisite = parse_url($this->buildPublicDlURL(), PHP_URL_PATH) ."/a.php";
        $server   = parse_url( Utils::detectServerURL(), PHP_URL_PATH) . dirname($_SERVER['REQUEST_URI']);
    ...
    function initPublicFolder($downloadFolder){
        ...
        @copy(APP_INSTALL_PATH."/server/index.html", $downloadFolder."/index.html");
        ...
        $content404 = str_replace(array("APP_MESSAGE_TITLE", "APP_MESSAGE_LEGEND"), array($sTitle, $sLegend), $content404);
    ...
    static function loadMinisite($data){
        ...
        $html = str_replace("APP_REPOSITORY_LABEL", ConfService::getRepositoryById($repository)->getDisplay(), $html);
        ...
        if(!empty($data["PRELOG_USER"])){
            ...
            $html = str_replace("APP_PRELOGED_USER", "", $html);
        }
        if(isSet($_GET["lang"])) {
            $loggedUser = &AuthService::getLoggedUser();
            if($loggedUser != null){
                ...
                setcookie("APP_lang", $_GET["lang"]);
            }
        }

        $tPath = (!empty($data["TRAVEL_PATH_TO_ROOT"]) ? $data["TRAVEL_PATH_TO_ROOT"] : "../..");
        $html = str_replace("APP_PATH_TO_ROOT", $tPath, $html);
    ...
    static function loadPubliclet($data)
    {
        ...
        foreach($images as $custom){
            ...
            if(!empty($confs[$varName])){
                if(strpos($confs[$varName], "plugins/") === 0 && is_file(APP_INSTALL_PATH."/".$confs[$varName])){
                    $realFile = APP_INSTALL_PATH."/".$confs[$varName];
        ...
        if (strlen($data["PASSWORD"]))
        {
            ...
            {
                $APP_LINK_HAS_PASSWORD = true;
                $APP_LINK_WRONG_PASSWORD = (isSet($_POST['password']) && ($_POST['password'] != $data["PASSWORD"]));
                include (APP_PLUGINS_FOLDER."/action.share/res/public_links.php");
                echo('<div style="position: absolute;z-index: 10000; bottom: 0; right: 0; color: #666;font-family: HelveticaNeue-Light,Helvetica Neue Light,Helvetica Neue,Helvetica,Arial,Lucida Grande,sans-serif;font-size: 13px;text-align: right;padding: 6px; line-height: 20px;text-shadow: 0px 1px 0px white;" class="no_select_bg"><br>Build your own shared box : <a style="color: #000000;" target="_blank" href="https://github.com/boa-project/boa">https://github.com/boa-project/boa</a></div>');
                return;
            }
        }else{
            if (!isSet($_GET["dl"])){
    ...
    function computeSharedRepositoryAccessRights($repoId, $mixUsersAndGroups, $currentFileUrl){
        ...
        foreach($groups as $gId => $gLabel){
            $r = AuthService::getRole("APP_GRP_".AuthService::filterBaseGroup($gId));
            if($r != null){
        ...
        foreach ($users as $userId => $userObject) {
            ...
            $uLabel = $userObject->personalRole->filterParameterValue("core.conf", "USER_DISPLAY_NAME", APP_REPO_SCOPE_ALL, "");
            ...
            if(!empty($ri)){
                ...
                if($this->watcher !== false){
                    $entry["WATCH"] = $this->watcher->hasWatchOnNode(
                        new ManifestNode($currentFileUrl),
                        $userId,
                        MetaWatchRegister::$META_WATCH_USERS_NAMESPACE
                    );
                }
    ...
    function createSharedRepository($httpVars, $repository, $accessDriver, $uniqueUser = null){
        ...
        while(isSet($httpVars["user_".$index])){
            ...
            if($eType == "user"){
                $u = Utils::decodeSecureMagic($httpVars["user_".$index], APP_SANITIZE_ALPHANUM);
                ...
                $u = Utils::decodeSecureMagic($httpVars["user_".$index]);
                $groups[] = $u;
            }
        ...
        if(isSet($editingRepo)){
            ...
            if(count($removeGroups)){
                foreach($removeGroups as $groupId){
                    $role = AuthService::getRole("APP_GRP_".AuthService::filterBaseGroup($groupId));
                    if($role !== false){
        ...
        foreach($users as $userName){
            ...
            if(isSet($httpVars["minisite"])){
                ...
                if(is_a($r, "BoA\Core\Security\Role")){
                    ...
                    if(isSet($allData["ACTIONS"][APP_REPO_SCOPE_SHARED])) $newData["ACTIONS"][$newRepo->getUniqueId()] = $allData["ACTIONS"][APP_REPO_SCOPE_SHARED];
                    if(isSet($allData["PARAMETERS"][APP_REPO_SCOPE_SHARED])) $newData["PARAMETERS"][$newRepo->getUniqueId()] = $allData["PARAMETERS"][APP_REPO_SCOPE_SHARED];
            ...
            if($this->watcher !== false){
                ...
                if($uWatches[$userName] == "true"){
                    $this->watcher->setWatchOnFolder(
                        new ManifestNode($this->urlBase.$file),
                        $userName,
        ...
        foreach($groups as $group){
            $grRole = AuthService::getRole("APP_GRP_".AuthService::filterBaseGroup($group), true);
    ...
    public static function deleteSharedElement($type, $element, $loggedUser){
        ...
        if($type == "repository"){
            ...
            if(!$repo->hasOwner() || $repo->getOwner() != $loggedUser->getId()){
                ...
                if($res == -1){
                    throw new \Exception($mess["boa_conf.51"]);
                }
            }
            ...
            if(!$repo->hasOwner() || $repo->getOwner() != $loggedUser->getId()){
                ...
                AuthService::deleteRole("APP_SHARED-".$repoId);
