...
Class.create("RoleEditor", AbstractEditor, {
    ...
    buildInfoPane : function(node, scope){
        ...
        if(scope == "user"){
            ...
            var defs = [
                $H({"name":"login",label:MessageHash["role_editor.21"],"type":"string", default:getBaseName(node.getPath()), readonly:true}),
                $H({"name":"profile",label:MessageHash["role_editor.22"],"type":"select", choices:profilesChoices, default:this.roleData.USER.PROFILE}),
                $H({"name":"default_repository",label:MessageHash["role_editor.23"],"type":"select", choices:repos.join(","),default:this.roleData.USER.DEFAULT_REPOSITORY}),
                $H({"name":"roles",label:MessageHash["role_editor.24"],"type":"select", multiple:true, choices:rolesChoicesString, default:this.roleData.USER.ROLES.join(",")})
            ];
            defs = $A(defs);
            ...
            b0.observe("click", function(){
                ...
                pane.insert(passEl1);
                var passEl2 = passEl1.cloneNode(true);passEl2.down("div").update(MessageHash["role_editor.30"]); passEl2.down("input").setAttribute("name", "pass_confirm");
                pane.insert(passEl2);
            ...
            b1.observe("click", function(){
                ...
                conn.onComplete = function(transport){
                    locked = !locked;
                    b1.update((locked?MessageHash["role_editor.27"]:MessageHash["role_editor.26"]));
                }.bind(this);
                conn.sendAsync();
            }.bind(this) );
            ...
            // MAIN INFO
            var defs = [
                $H({"name":"roleId",label:MessageHash["role_editor.31"],"type":"string", default:getBaseName(node.getPath()), readonly:true}),
                $H({"name":"applies",label:MessageHash["role_editor.33"],"type":"select", multiple:true, choices:"standard|All users,admin|Administrator,shared|Shared,guest|Guest"})
            ];
            defs = $A(defs);
            f.createParametersInputs(this.element.down("#pane-infos").down("#account_infos"), defs, true, false, false, true);
            ...
            // MAIN INFO
            var defs = [
                $H({"name":"groupId",label:MessageHash["role_editor.34"],"type":"string", default:getBaseName(node.getPath()), readonly:true}),
                $H({"name":"groupLabel",label:MessageHash["role_editor.35"],"type":"string", default:this.roleData.GROUP.LABEL})
            ];
            defs = $A(defs);
            f.createParametersInputs(this.element.down("#pane-infos").down("#account_infos"), defs, true, false, false, true);
        ...
        definitions.each(function(paramObject){
            ...
            if(param.get("name").endsWith("DISPLAY_NAME") && param.get("default")){
                ...
                if(this.roleData.USER && this.roleData.USER.LOCK) display += " ("+ MessageHash["role_editor.36"] +")";
        ...
        if(!updatedDefs.length){
            ...
            if(scope == "role"){
                this.element.down("#pane-infos").down("#account_custom").previous("div.innerTitle").update(MessageHash["role_editor.42"]);
            }
    ...
    feedPluginsSelectors : function(jsonData, select){
        ...
        select.observe("change", function(){
            ...
            nextSelect.insert(new Element("option", {value:-1}).update((type == "action" ? MessageHash["role_editor.12a"]:MessageHash["role_editor.12b"])));
            ...
            nextSelect.observeOnce("change", function(){
                ...
                lastSelect.focus();
                lastSelect.down("option").update(MessageHash["role_editor.12c"]);
    ...
    generateRightsTable : function(){
        ...
        //var defaultRepository = XPathGetSingleNodeText(xmlData, '//pref[@name="force_default_repository"]/@value');
   		for(var repoId in repositories){
            ...
            rightsCell.insert('<label for="chck_'+repoId+'_read">' + MessageHash['boaconf.29'] + '</label> ');
            rightsCell.insert(writeBox);
            rightsCell.insert('<label for="chck_'+repoId+'_write">' + MessageHash['boaconf.30'] + '</label> ');
            ...
            for(var pluginId in actionsData[repoScope]){
                for(var actionName in actionsData[repoScope][pluginId]){
                    ...
                    if(this.isInherited(['ACTIONS', repoScope, pluginId, actionName])){
                        el.insert(" ("+MessageHash["role_editor.38"]+")");
        ...
        conn.onComplete = function(transport){
            ...
            for(var i=0;i<scopes.length;i++){
                ...
                for(var k=0;k<formParams.length;k++){
                    ...
                    if(this.isInherited(["PARAMETERS", id, h.get("group"), h.get("name")])){
                        h.set("label", '<span class="inherited">' + h.get("label") + ' ('+ MessageHash["role_editor.38"] +')' + '</span>');
                    }
                }
            ...
            parametersPane.select("div.SF_element").each(function(element){
                if(!element.down("span.inherited")){
                    var removeLink = new Element("span", {className:"list_remove_item"}).update(MessageHash["role_editor.41"]);
