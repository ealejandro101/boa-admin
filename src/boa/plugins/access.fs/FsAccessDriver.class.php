	...
	*/
	...
	function switchAction($action, $httpVars, $fileVars){
		...
		{			
			...
			//------------------------------------
			case "download":
				BoALogger::logAction("Download", array("files"=>$selection));
			...
			case "get_content":
                ...
                BoALogger::logAction("Get_content", array("files"=>$selection));
			...
			case "put_content":	
				...
				BoALogger::logAction("Online Edition", array("file"=>$file));
			...
			//throw new ApplicationException("", 113);
				...
				if(count($error)){					
                    ...
                    if(isSet($httpVars["force_copy_delete"])){
                        ...
                        BoALogger::logAction("Copy/Delete", array("files"=>$selection, "destination" => $dest));
                    }else{
                        BoALogger::logAction(($action=="move"?"Move":"Copy"), array("files"=>$selection, "destination"=>$dest));
                    }
                    $logMessage = join("\n", $success);
				}
                if(!isSet($nodesDiffs)) $nodesDiffs = $this->getNodesDiffArray();
			...
			case "delete";
				...
				if($errorMessage) throw new ApplicationException(SystemTextEncoding::toUTF8($errorMessage));
				BoALogger::logAction("Delete", array("files"=>$selection));
                if(!isSet($nodesDiffs)) $nodesDiffs = $this->getNodesDiffArray();
			...
			case "rename";
				...
				BoALogger::logAction("Rename", array("original"=>$file, "new"=>$filename_new));
			...
			case "mkdir";
                ...
                BoALogger::logAction("Create Dir", array("dir"=>$dir."/".$dirname));
			...
			case "mkfile";
				...
				BoALogger::logAction("Create File", array("file"=>$dir."/".$filename));
			...
			case "chmod";
                ...
                BoALogger::logAction("Chmod", array("dir"=>$dir, "filesCount"=>count($changedFiles)));
                if(!isSet($nodesDiffs)) $nodesDiffs = $this->getNodesDiffArray();
			...
			case "upload":
				...
				BoALogger::debug("Upload Files Data", $fileVars);
				$destination=$this->urlBase.Utils::decodeSecureMagic($dir);
				BoALogger::debug("Upload inside", array("destination"=>$destination));
				if(!$this->isWriteable($destination))
				{
					$errorCode = 412;
					$errorMessage = "$mess[38] ".SystemTextEncoding::toUTF8($dir)." $mess[99].";
					BoALogger::debug("Upload error 412", array("destination"=>$destination));
				...
				foreach ($fileVars as $boxName => $boxData)
				{
                    ...
                    BoALogger::debug("User filename ".$userfile_name);
					...
					if(isSet($boxData["input_upload"])){
						try{
							BoALogger::debug("Begining reading INPUT stream");
							...
							fclose($input);
							fclose($output);
							BoALogger::debug("End reading INPUT stream");
						}catch (Exception $e){
							$errorCode=411;
							$errorMessage = $e->getMessage();
							break;
                    ...
                    if(isSet($httpVars["appendto_urlencoded_part"])){
                        ...
                        if(file_exists($destination ."/" . $appendTo)){
                            BoALogger::debug("Should copy stream from $userfile_name to $appendTo");
                            ...
                            fclose($partO);
                            fclose($appendF);
                            BoALogger::debug("Done, closing streams!");
                        }
					...
					BoALogger::logAction("Upload File", array("file"=>SystemTextEncoding::fromUTF8($dir)."/".$userfile_name));
				}
				
				if(isSet($errorMessage)){
					BoALogger::debug("Return error $errorCode $errorMessage");
					return array("ERROR" => array("CODE" => $errorCode, "MESSAGE" => $errorMessage));
				}else{
					BoALogger::debug("Return success");
			...
			case "ls":
				...
				BoALogger::debug("LS Time : ".intval((microtime()-$startTime)*1000)."ms");
	...
	function readFile($filePathOrData, $headerType="plain", $localName="", $data=false, $gzip=null, $realfileSystem=false, $byteOffset=-1, $byteLength=-1)
	{
		...
		else
		{
            ...
            */
			if ($isFile) {
				...
				BoALogger::debug("Sending accept range 0-$size");
			}
			...
			if ( isset($_SERVER['HTTP_RANGE']) && $isFile && $size != 0 )
			{
				if($headerType == "stream_content"){
					if(extension_loaded('fileinfo')  && $this->wrapperClassName == "fsAccessWrapper"){
            			...
            			BoALogger::debug("Detected mime $mimeType for $realfile");
					}else{
				...
				BoALogger::debug('Content-Range: bytes ' . $offset . '-' . $length . '/' . $size);
				...
				{
					BoALogger::debug("dl reading $readSize to $length", $_SERVER["HTTP_RANGE"]);					
		...
		if($data){
			...
			if($realfileSystem){
				BoALogger::debug("realFS!", array("file"=>$filePathOrData));
	...
	function extractArchive($destDir, $selection, &$error, &$success){
		...
		BoALogger::debug("Archive", $files);
		$realDestination = call_user_func(array($this->wrapperClassName, "getRealFSReference"), $this->urlBase.$destDir);
		BoALogger::debug("Extract", array($realDestination, $realZipFile, $files, $zipLocalPath));
	...
	{
		BoALogger::debug("CopyMove", array("dest"=>$destDir, "selection" => $selectedFiles));
		$mess = ConfService::getMessages();
    ...
    function makeZip ($src, $dest, $basedir)
    {
    	...
    	BoALogger::debug("Pathes", $filePaths);
    	BoALogger::debug("Basedir", array($basedir));
        ...
        while (strlen($file = readdir($handle)) > 0)
        {
            ...
            if(is_file($dirName."/".$file)){
                ...
                if( $docAge > $purgeTime){
                    ...
                    BoALogger::logAction("Purge", array("file" => $dirName."/".$file));
