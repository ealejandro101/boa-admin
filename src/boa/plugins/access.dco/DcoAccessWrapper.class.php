...
use BoA\Core\Access\FileWrapper;
 ...
 * @subpackage Access
 */
class DcoAccessWrapper implements FileWrapper {
            ...
            if($streamType == "file"){
                if(self::$crtZip == null ||  !is_array(self::$currentListingKeys)){
                    ...
                    if(array_key_exists($key, self::$currentListing)){
                        ...
                        throw new ApplicationException("Cannot find key");
                    }
                }
            }else{
                $crtZip = new PclZip(Utils::securePath(realpath($repoObject->getOption("PATH")).$repoObject->resolveVirtualRoots($zipPath)));
    ...
    public function stream_stat(){
        ...
        if(is_resource($this->fp)){
            ...
            DcoAccessWrapper::$lastRealSize = false;
            if($statValue[2] > 0 && $PROBE_REAL_SIZE && !ini_get("safe_mode")){
                DcoAccessWrapper::$lastRealSize = floatval(trim($this->getTrueSizeOnFileSystem($this->realPath)));
            }
            return $statValue;
        }
