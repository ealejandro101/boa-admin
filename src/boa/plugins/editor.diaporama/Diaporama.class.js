...
Class.create("Diaporama", AbstractEditor, {
	...
	initialize: function($super, oFormObject, options)
	{
        options = Object.extend({
            ...
            actions : (window.appMinisite || window.appMobile) ? {} : {
                'toggleSideBar' : '<a id="toggleButton"><img src="'+resourcesFolder+'/images/actions/22/view_left_close.png"  width="22" height="22" alt="" border="0"><br><span message_id="86"></span></a>'
            }
        }, options);
		...
		var inputStyle = {
			backgroundImage:'url("'+resourcesFolder+'/images/locationBg.gif")',
		...
		this.baseUrl = _bootstrap.parameters.get('appServerAccess')+'&action=preview_data_proxy&file=';
        ...
        if(window.app.actionBar.getActionByName("download")){
            this.downloadButton.onclick = function(){
                if(!this.currentFile) return;
                app.triggerDownload(_bootstrap.parameters.get('appServerAccess')+'&action=download&file='+encodeURIComponent(this.currentFile));
                return false;
            }.bind(this);
        }else{
		...
		if(app && app.user){
			...
			if(autoFit && autoFit == "true"){
				this.autoFit = true;
				this.fitToScreenButton.select('img')[0].src = resourcesFolder + '/images/actions/22/zoom-fit-restore.png';
	...
	toggleFitToScreen:function(skipSave){
		...
		this.fitToScreenButton.select('img')[0].src = resourcesFolder + '/images/actions/22/'+src+'.png';
	 ...
	 * @returns {___img1}
	 */
	getPreview : function(node){
		var img = new Element('img', {
            src:Diaporama.prototype.getThumbnailSource(node),
            className:'thumbnail_iconlike_shadow',
            align:"absmiddle"
        });
        if(!parseInt(node.getMetadata().get("image_width"))){
            var imgObject = new Image();
            imgObject.onload = function(){
                img.DIMENSIONS_LOADING = false;
                node.getMetadata().set("image_dimensions_thumb", true);
                node.getMetadata().set("image_width", this.width);
                node.getMetadata().set("image_height", this.height);
            }
            ...
            imgObject.src = Diaporama.prototype.getThumbnailSource(node);
        }
	...
	getThumbnailSource : function(node){
        var repoString = "";
        if(app.repositoryId && node.getMetadata().get("repository_id") && node.getMetadata().get("repository_id") != app.repositoryId){
            repoString = "&tmp_repository_id=" + node.getMetadata().get("repository_id");
        }
		var source = appServerAccessPath + repoString + "&get_action=preview_data_proxy&get_thumb=true&file="+encodeURIComponent(node.getPath());
		if(node.getParent()){
            var preview_seed = node.getParent().getMetadata().get('preview_seed');
    		if(preview_seed){
