...
Class.create("VideoPreviewer", AbstractEditor, {
    ...
    open : function($super, node){
        this.currentRichPreview = this.getPreview(node, true);
	...
	getPreview : function(node, rich){
		if(rich){
            ...
            if(mime == "mp4" || mime == "m4v" || mime == "webm" || mime == "ogv"){
                html5proxies.set(mime, node.getPath());
            }
            ...
            if(html5proxies.keys().length){
				...
				if(!window.crtAppSessid){
					var connexion = new Connexion();
					connexion.addParameter("get_action", "get_sess_id");
					connexion.onComplete = function(transport){
						window.crtAppSessid = transport.responseText.trim();
						window.setTimeout(function(){window.crtAppSessid = null}, 1000 * 60 * 5);
					};
					connexion.sendSync();
				}
				var sessidPart = '&app_sessid='+window.crtAppSessid;
				...
				var poster = resolveImageSource(node.getIcon(),'/images/mimes/ICON_SIZE',64);
                ...
                html5proxies.each(function(pair){
                    var fname = url+'/'+_bootstrap.parameters.get('appServerAccess')+'&action=read_video_data'+sessidPart+'&file='+pair.value;
                    if(!flashName){
                ...
                var f = encodeURIComponent(url+'/'+_bootstrap.parameters.get('appServerAccess')+'&action=read_video_data&file='+node.getPath());
			...
			return new Element('img', {src:resolveImageSource(node.getIcon(),'/images/mimes/ICON_SIZE',64)});
		}
	},
