...
Class.create("RepositoryEditor", AbstractEditor, {
    ...
    feedRepositoryForm: function(xmlData, metaTab){
        ...
        var driverParams = XPathSelectNodes(xmlData, "admin_data/coredriver/param");
        ...
        if(!tplParams.length){
            if(this.currentRepoWriteable){
                ...
                this.metaPane.update(MessageHash['repository_editor.15']);
            }
        }
    ...
    feedMetaSourceForm : function(xmlData, metaPane){
        ...
        if(data && data.firstChild && data.firstChild.nodeValue){
            ...
            for(var plugId in metaSourcesData){
                ...
                var saveButton = null;
                if(insertSave){
                    accordionContent.insert("<div tabindex='0' name='meta_source_edit' class='largeButton SF_disabled' style='min-width:70px;clear:both;margin-top: 7px;margin-left: 0'><img src=\""+resourcesFolder+"/images/actions/16/filesave.png\"><span class=\"title\">Save</span></div>");
                    saveButton = accordionContent.down("div[name='meta_source_edit']");
                }
                accordionContent.insert("<div  tabindex='0' name='meta_source_delete' class='largeButton' style='min-width:70px;clear:both;margin-top: 7px;margin-left: 0'><img src=\""+resourcesFolder+"/images/actions/16/editdelete.png\"><span class=\"title\">Remove</span></div>");
        ...
        var formEl = new Element("div", {className:"SF_element"}).update("<div class='SF_label'>"+MessageHash['repository_editor.12']+" :</div>");
        ...
        for(var i=0;i<choices.length;i++){
            ...
            if(!currentGroup || type != prevType){
                currentGroup = new Element("optgroup", {label:MessageHash["repository_editor.9"].replace("%s", type)});
        ...
        addForm.insert({before: new Element("div", {className:"innerTitle"}).update(MessageHash["repository_editor.5"])});
        addForm.insert({before: new Element("div", {className:"dialogLegend"}).update(MessageHash["repository_editor.7"])});
        addForm.insert({after : new Element("div", {className:"dialogLegend"}).update(MessageHash["repository_editor.8"])});
        addForm.insert({after : new Element("div", {className:"innerTitle"}).update(MessageHash["repository_editor.6"])});
        ...
        this.metaSelector.observe("change", function(){
            ...
            modal.refreshDialogAppearance();
            modal.refreshDialogPosition();
            addFormDetail.insert("<div class='largeButton' style='width:100px;clear:both;margin-top: 7px;margin-left: 0'><img src=\""+resourcesFolder+"/images/actions/16/filesave.png\"><span class=\"title\">"+MessageHash['repository_editor.11']+"</span></div>");
    ...
    metaActionClick : function(event){
        ...
        if(params.get('get_action') == 'meta_source_add' && params.get('DRIVER_OPTION_new_meta_source') == ''){
            alert(MessageHash['repository_editor.14']);
            return;
        }
        ...
        if(params.get('get_action') == 'meta_source_delete'){
            var res = confirm(MessageHash['repository_editor.13']);
            if(!res) return;
        }
