...
"Main container for core System settings (application title, sharing, webdav server config, etc...)"=>"Simple container pour l'édition des options principales d'[Application].",
...
"Download URL" => "URL",
"If not inferred directly from the current URI plus the public download folder name, replace the public access URL here." => "Si cette URL n'est pas logiquement déduite de l'URL d'[Application] (si le répertoire est dans un virtual host par exemple), entrez l'URL ici.",
...
"Use framework via the command line, allowing CRONTAB jobs or background actions." => "Utiliser le framework [Application] via la ligne de commande, ce qui permet de programmer des jobs CRONTAB ou des actions en tâche de fond",
...
"Try to use the current System user credentials for connecting. Warning, the APP_SESSION_SET_CREDENTIALS config must be set to true!" => "Utilisation des identifiants de la session, entrés via le formulaire de login. 'Set Session Credentials' doit être activé dans les préférences générales d'[Application].",
...
"Prefilled label for the new repository, you can use the APP_USER keyworkd in it." => "Libellé prérempli pour le nouveau dépôt, vous pouvez utiliser APP_USER.",
