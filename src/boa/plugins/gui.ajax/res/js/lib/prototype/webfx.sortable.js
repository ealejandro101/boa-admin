...
SortableTable = Class.create({
	...
	getHeaderCells : function(){
		if(!this.headerCells && this.tHead){
			...
			this.headerCells.each(function(cell){
                ...
                if(!cell.down('span.sort-icon')){
                    cell.down('div.header_label').insert('<span class="sort-icon icon-caret-up"></span><span class="sort-icon icon-caret-down"></span>');
                }
			});
		}
	...
	onHeaderResize : function(headerCells){
		...
		var repoData = data.get(app.user.getActiveRepository());
		...
		data.set(app.user.getActiveRepository(), repoData);
