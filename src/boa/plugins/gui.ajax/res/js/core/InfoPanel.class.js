...
Class.create("InfoPanel", AppPane, {
        ...
        this.contentContainer.select('[data-action]').each(function(act){
            if(act.getAttribute('data-action') != 'no-action'){
                act.observe('click', function(event){
                    window.app.actionBar.fireAction(event.target.getAttribute('data-action'));
                }.bind(this));
            }else{
    ...
    updateTitle : function(title){
        ...
        if(panelTitle) {
            if(panelTitle.down('span[message_id]')) panelTitle.down('span[message_id]').update(title);
    ...
    contributePanelHeaderIcon:function(iconClass, iconTitle, action, panelPointer){
        ...
        if(action){
            ...
            ic.observe("click", function(){
                app.actionBar.fireAction(action);
            }.bind(this));
        }
        if(panelPointer){
