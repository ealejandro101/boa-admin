...
/**
 * The godzilla of the app, should be split in smaller pieces.. 
...
Class.create("FilesList", SelectableElements, {
    ...
    initialize: function($super, oElement, initDefaultDispOrOptions)
    {
        ...
        var selectionChangedObserver = function(event){
            ...
            var dm = (this._dataModel?this._dataModel:app.getContextHolder());
        ...
        if(this._dataModel){
            ...
            this._registerObserver(document, "app:context_changed", contextChangedObserver, false );
            this._registerObserver(document, "app:context_loading", loadingObs, false);
            this._registerObserver(document, "app:component_config_changed", componentConfigObserver, false);
            this._registerObserver(document, "app:selection_changed", selectionChangedObserver, false);
        }
    ...
    parseComponentConfig : function(domNode){
        ...
        if(properties.length){
            for( var i=0; i<properties.length;i++){
                ...
                else if(property.getAttribute("name") == "detailThumbSize"){
                    var value = parseInt(property.getAttribute("value"));
                    this._detailThumbSize = isNaN(value) ? this._detailThumbSize : value;
                    refreshGUI = true;
                }
                else if(property.getAttribute("name") == "fixedDisplayMode"){
                    this.fixedDisplayMode = property.getAttribute("value");
                    refreshGUI = true;
                }
            }
        }
    ...
    getActions : function(){
        ...
        var options2 = {
            ...
            listeners : {
                init:function(){
                    var actBar = window.app.actionBar;
    ...
    getCurrentContextNode : function(){
        ...
        else return app.getContextNode();
    },
    ...
    resize : function(){
        ...
        this.notify("resize");
        document.fire("app:resize-FilesList-" + this.htmlElement.id, this.htmlElement.getDimensions());
    },
    ...
    fill: function(contextNode){
        ...
        this.gridStyle = 'file';
        this.even = false;
        this._oSortrrs = this.defaultSortTypes;
    ...
    addInlineToolbar : function(element, node){
        if(this._inlineToolbarOptions){
            ...
            aT.actions = app.actionBar.actions;
            aT.initToolbars();
            if(!this._inlineToolbarOptions.unique){
                var dm = (this._dataModel?this._dataModel:app.getContextHolder());
    ...
    resizeThumbnails: function(one_element){
        ...
        elList.each(function(element){
            ...
            if(element.down('div.thumbnail_selectable_cell')){
                ...
                element.setStyle({minHeight: (tSize+20)+'px'})
            }else{
    ...
    selectFile: function(fileName, multiple)
    {
        ...
        if(!app.getContextHolder().fileNameExists(fileName, true))
        {
            return;
        }
        var allItems = this.getItems();
