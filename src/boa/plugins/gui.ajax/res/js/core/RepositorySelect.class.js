...
Class.create("RepositorySelect", {
	...
	refreshRepositoriesMenu: function(repositoryList, repositoryId){
        ...
        var menuItems = $A();
        if(!this.loadedOnce) {
            app.actionBar.loadActionsFromRegistry(app.getXmlRegistry());
            this.loadedOnce = true;
        }
        var otherActions = app.actionBar.getActionsForAppWidget("RepositorySelect", this.element.id).each(function(otherAction){
            menuItems.push({
