...
Class.create("Action", {
    ...
    fireSelectionChange: function(){
        ...
        if(selectionContext.custom){
            var result = false;
            try{
                result = eval(selectionContext.custom);    
            }
            catch(err){}
            if(!result){
                if(selectionContext.behaviour == 'hidden') this.hide();
     ...
     */
    hide: function(){       
        this.deny_before = this.deny;
        this.deny = true;
        this.notify('hide');
    },
    
    /**
     * Reset hide state to previus value
     * Notifies "hide" Event
     */
    resetHide: function(){       
        this.deny = this.deny_before || this.deny;
        this.notify('hide');
    },
